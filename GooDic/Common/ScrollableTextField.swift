...
//            else if widthToCursor - scrollView.contentOffset.x > scrollView.bounds.width * 0.5 { // cursor on the right --> let its position move to the center.
    ...
    private func updateWidthWhenReplace() {
        guard let hookedTF = self.textField, let fullWidth = hookedTF.getWidthFromDocumentBeginingToEnd()   else {
            return
        }
